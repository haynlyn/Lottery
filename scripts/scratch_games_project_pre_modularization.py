...
def main():
    ...
    # make dataframe with game ID, metrics, and date of data collected
    match_object_of_fp = reg.match(r'(\D+)(\d+)(.*)', json_filepath)
    date_of_data = match_object_of_fp.group(2)
    games_to_study = set(games.keys()).intersection(set(num_price_dic.keys())).intersection(set(num_name_dic))
    data = []
    for game_id in games_to_study:
        observation = measure(games[game_id], num_price_dic[game_id])
        observation['id'] = game_id
        observation['price'] = num_price_dic[game_id]
        data.append(observation)
    df = pd.DataFrame(data = data)
    df['date'] = date_of_data
    metrics = [x for x in df.columns if x not in ['price', 'id', 'date']]
    ideal = df.iloc[df[metrics].idxmax(axis = 'rows')].drop_duplicates()
    print(ideal)
